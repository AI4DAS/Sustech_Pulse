...
answer = pd.read_csv("/home/disk/disk02/wzm/Sustech_Pulse/tutorial/answer07/T3.an",sep='\s+' ,names=['filename','class','name'])
score=0
y=[]
...
#    print(class_out, answer.iloc[i,1]) 
   if int(class_out)==int(answer.iloc[i,1]):
       ...
       y.append( [i, class_out , answer.iloc[i,1], answer.iloc[i,2] ])
       

print("cls score",score)
y = pd.DataFrame(y, columns=['index','output','class','name'])
print(y)
