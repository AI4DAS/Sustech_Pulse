...
def DiTing_EQDet_phase_TTA_predict(input_st, dpk_device, dpk_model,  window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
...
def DiTing_EQDet_phase_TTA_predict_2(input_st, dpk_device, dpk_model,  window_length=10000, step_size=10000, p_th=0.3, s_th=0.5, det_th=0.50):
    input_st.detrend('demean') #去均值

    input_st.detrend('linear') #去趋势
    print(len(input_st[0]))
    filter_bank = [(0.05 ,  20.0),(0.005,50.0)]
    filter_P = []
    filter_S = []
    resample_bank = [80 , 100]
    for resample_rate in resample_bank:
        input_st.resample(resample_rate)
        print('Resample: {} Hz'.format(resample_rate))
