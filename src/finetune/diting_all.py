...
sys.path.append("/home/disk/disk02/wzm/Sustech_Pulse/src/finetune/wzm")
from dpk_utils import DiTing_EQDet_PhasePick_predict_fastV2
...
def DiTing_EQDet_Classify_predict(stream, window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
    data_len = stream[0].data.shape[0]
    ...
    for i in tqdm(range(num_windows)):
        ...
        try:
            ...
            print("响应错误：", response.text)
        output_class = np.argmax(output_np[0])+1
    return output_class


def DiTing_EQDet_magnitude_predict(stream, window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
    data_len = stream[0].data.shape[0]
    ...
    for i in tqdm(range(num_windows)):
        ...
        try:
            output_np = np.array(response.json()['result'])
            out = round(output_np[0,0],2)
        except Exception as e:
            print("响应错误：", response.text)
    return out


def DiTing_EQDet_phase_TTA_predict(stream, dpk_device, dpk_model,  window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
    filter_bank = [(1, 45), (1, 30), (1, 20), (0.5, 40), (1.5, 45),(0.05 , 20.0),(0.005,50.0)]
    filter_P = []
    filter_S = []
    
    for f, w in filter_bank:
        input_st = stream.copy()
        input_st.filter('bandpass', freqmin=f, freqmax=w, corners=4, zerophase=True)
        # print('Filter: freqmin={}, freqmax={}'.format(f, w))
        events = DiTing_EQDet_PhasePick_predict_fastV2(input_st, dpk_device, dpk_model, window_length, step_size, p_th, s_th, det_th)
        for t_event in events:
            try:
                filter_P.append(t_event[1][0][0]/100.0)
            except:
                pass
            try:
                filter_S.append(t_event[2][0][0]/100.0)
            except:
                pass

    
   
    resample_bank = [80, 120, 140, 160, 180, 200]
    for resample_rate in resample_bank:
        input_st = stream.copy()
        input_st.resample(resample_rate)
        # print('Resample: {} Hz'.format(resample_rate))
        events = DiTing_EQDet_PhasePick_predict_fastV2(input_st, dpk_device, dpk_model, window_length, step_size, p_th, s_th, det_th)
        for t_event in events:
            try:
                filter_P.append(t_event[1][0][0]/float(resample_rate))
            except:
                pass
            try:
                filter_P.append(t_event[2][0][0]/float(resample_rate))
            except:
                pass
    
    final_P = np.mean(filter_P)
    final_S = np.mean(filter_S)

    return final_P,final_S


def DiTing_EQDet_phase_TTA_predict_3(stream, dpk_device, dpk_model,  window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.10):
    stream.detrend('demean') #去均值

    stream.detrend('linear') #去趋势
    # print(len(stream[0]))
    filter_bank = [(1, 45), (1, 30), (1, 20), (0.5, 40), (1.5, 45),(0.05 , 20.0),(0.005,50.0)]
    resample_bank = [80,100, 120, 140, 160, 180, 200]
    ...
    for resample_rate in resample_bank:
        ...
        re_stream.resample(resample_rate)
        # print('Resample: {} Hz'.format(resample_rate))
        for f, w in filter_bank:
            input_st = re_stream.copy()
            ...
            for t_event in events:
                try:
                    if t_event[1][0][1] >= con_P:
                        best_P = t_event[1][0][0]/ float(resample_rate)
                except:
                    pass
                try:
                    if t_event[2][0][1] >= con_S:
                        best_S = t_event[2][0][0]/ float(resample_rate)
                except:
                    pass
    if best_P == 0:
        print("tta 3 no P out")
    if best_S == 0:
        print("tta 3 no S out")
    return best_P,best_S


def DiTing_EQDet_phase_TTA_predict_2(stream, dpk_device, dpk_model,  window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.30):
    stream.detrend('demean') #去均值

    stream.detrend('linear') #去趋势
    # print(len(stream[0]))
    filter_bank = [(0.05 ,  20.0) ]
    resample_bank = [ 100,80]
        ...
        for f, w in filter_bank:
            ...
            for t_event in events:
                try:
                    ...
                    pass
                try:
                    filter_S.append(t_event[2][0][0]/float(resample_rate))
                except:
                    pass
    final_P = np.median(filter_P)
    final_S = np.median(filter_S)

    if len(filter_P) == 0:
        print("tta 2 no P out")
    if len(filter_S) == 0:
        print("tta 2 no S out")
    
    if np.isnan(final_S) :
        final_S = final_P
    if np.isnan(final_S) and np.isnan(final_P):
        final_P,final_S =  DiTing_EQDet_phase_TTA_predict_3(stream, dpk_device, dpk_model)
    return final_P,final_S

def DiTing_EQDet_phase_TTA_predict_4(stream, dpk_device, dpk_model,  window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
    stream.detrend('demean') #去均值
...
def DiTing_EQDet_magnitude_predict_4_7(stream, window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
    data_len = stream[0].data.shape[0]
...
def DiTing_EQDet_magnitude_predict_perturbation(stream, window_length=10000, step_size=10000, p_th=0.1, s_th=0.1, det_th=0.50):
    data_len = stream[0].data.shape[0]
    ...
    #扰动函数random.randint(0, 1000)
    for i in range(0,data_len):
        tmp_waveform[i, 0] += random.randint(-500, 500)
        tmp_waveform[i, 1] += random.randint(-500, 500)
        tmp_waveform[i, 2] += random.randint(-500, 500)
